...
- **FR-009**: The system MUST apply seating constraints in this order: structural validity, mandatory couple placement, explicit guest separation rules, explicit companion rules, location preferences, explicit conversation-volume avoidance, explicit named social avoidance, explicit food/drink or shared-topic affinities, then group-membership preferences.
...
- **FR-017**: The system MUST use a documented soft avoidance score to disperse guests whose descriptions explicitly identify high-volume, confrontational, or debate-prone communication. Shared communication traits MUST NOT create a hard separation unless an unambiguous narrative reference names the other guest.
- **FR-018**: The system MUST use a documented soft avoidance score for explicitly named teasing or conflict and MAY use an explicit shared topic as a soft affinity. Neither rule may override structural, mandatory, companion, or separation constraints.
...
- **Seating Policy**: A documented, evidence-backed rule that expresses a mandatory placement, separation, companion, location preference, conversation-avoidance preference, social avoidance preference, or affinity and its priority.
